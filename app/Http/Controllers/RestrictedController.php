...
class RestrictedController extends Controller
{
    ...
    private function getCurrentEventId(){
        $currentEvent = $this->user->current_event()->get();
    ...
    private function setLogo(){
        $logos = [
            "logo200" => asset('img/logoo2200.png'),
            "logo50" => asset('img/logoo250.png'),
        ];
    ...
    private function makeMenu(){
        $userGroup = $this->user->group()->get();
        $groupMenu = $userGroup[0]->menu();
    ...
    private function checkModule($request){
        $userGroup = $this->user->group()->get();
        $allowedModules = $userGroup[0]->modules()->get();

        $continue = false;
        foreach ($allowedModules as $key => $value) {
            if(empty($value->father_path)) //Check group permission to father module
            {
                if($request->is($value->path . "*")){
                    $continue = true;
                }                
            }
        }
